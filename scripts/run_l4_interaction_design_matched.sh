#!/usr/bin/env bash
# Run the L4 interaction-design smoke across a matched-model × modality grid.
#
# Structurally identical to run_l4_business_alignment_matched.sh — only the
...
SMOKE="scripts/smoke_l4_interaction_design_multimodal.py"
...
for model in "${MODELS[@]}"; do
    ...
    for modality in "${MODALITIES[@]}"; do
        ...
        target="data/derived/l4_audit/audit_interaction_design/l4_verdicts_audit_interaction_design_cluster02${suffix}.provenance.json"
...
ls -1 data/derived/l4_audit/audit_interaction_design/*.provenance.json | sort
