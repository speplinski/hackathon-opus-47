"""One-shot smoke for the L4 interaction-design audit — text or multimodal.

Companion to the production ``l4_audit_interaction_design`` module
(Cooper's About Face lens — posture, excise, idioms, etiquette).
...
``smoke_l4_business_alignment_multimodal.py`` — only the module
import and provenance shape (posture histogram + user-tier histogram
+ excise-type histogram + mixed-posture / excise gauges instead of
building-block + pattern + tension fields) differ.
...
from auditable_design.layers.l4_audit_interaction_design import (  # noqa: E402
    DIMENSION_KEYS,
    MAX_TOKENS,
    SKILL_ID,
    SYSTEM_PROMPT,
...
DEFAULT_OUT_DIR = _REPO_ROOT / "data/derived/l4_audit/audit_interaction_design"
...
def _build_provenance(
    *,
    ...
    smoke vs. prod without adapter code. Cooper-specific: posture
    histogram (7 keys including not_applicable) + user-tier histogram
    (4 keys) + excise-type histogram (5 keys) + mixed-posture /
    excise gauges, instead of block/pattern/tension fields.
    """
    ...
    posture_hist: dict[str, int] = {p: 0 for p in VALID_POSTURES}
    tier_hist: dict[str, int] = {t: 0 for t in VALID_USER_TIERS}
    excise_hist: dict[str, int] = {e: 0 for e in VALID_EXCISE_TYPES}
    mixed_posture_findings = 0
    excise_findings = 0
    ...
    if payload is not None:
        ...
        for f in payload["findings"]:
            ...
            posture = f["posture"]
            posture_hist[posture] = posture_hist.get(posture, 0) + 1
            tier = f["user_tier"]
            tier_hist[tier] = tier_hist.get(tier, 0) + 1
            excise = f["excise_type"]
            excise_hist[excise] = excise_hist.get(excise, 0) + 1
            if posture == "mixed":
                mixed_posture_findings += 1
            if excise != "none":
                excise_findings += 1
    ...
    return {
        ...
        "posture_histogram": posture_hist,
        "user_tier_histogram": tier_hist,
        "excise_type_histogram": excise_hist,
        "mixed_posture_findings": mixed_posture_findings,
        "excise_findings": excise_findings,
        "fallback_reasons": (
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Smoke for L4 audit-interaction-design on one enriched cluster. "
    ...
    native_stem = f"l4_verdicts_audit_interaction_design_{cluster_stem}{suffix}"
