"""One-shot smoke for the L4 business-alignment audit — text or multimodal.

Companion to the production ``l4_audit_business_alignment`` module.
...
``smoke_l4_decision_psychology_multimodal.py`` — only the module
import and provenance shape (building-block histogram + pattern
histogram + tension-pair counter + tension/single-block gauges
instead of intent histogram + mechanism counts) differ.
...
from auditable_design.layers.l4_audit_business_alignment import (  # noqa: E402
    DIMENSION_KEYS,
    MAX_TOKENS,
    SKILL_ID,
    SYSTEM_PROMPT,
...
DEFAULT_INPUT = (
    _REPO_ROOT
    / "data/derived/l4_audit/audit_business_alignment/audit_business_alignment_input.jsonl"
)
DEFAULT_SCREENSHOT = _REPO_ROOT / "data/artifacts/ui/duolingo_streak_modal.png"
DEFAULT_OUT_DIR = _REPO_ROOT / "data/derived/l4_audit/audit_business_alignment"
...
def _build_provenance(
    *,
    ...
    smoke vs. prod without adapter code. Osterwalder-specific:
    per-block histogram + pattern histogram + tension-pair counter
    + tension/single-block gauges, instead of intent/mechanism fields.
    """
    ...
    block_counts: dict[str, int] = {b: 0 for b in VALID_BLOCKS}
    pattern_hist: dict[str, int] = {p: 0 for p in VALID_PATTERNS}
    tension_counter: Counter[tuple[str, str]] = Counter()
    tension_findings = 0
    single_block_findings = 0
    ...
    if payload is not None:
        ...
        for f in payload["findings"]:
            ...
            if tension:
                tension_findings += 1
                tension_counter[(tension[0], tension[1])] += 1
            else:
                single_block_findings += 1

    # Stable-sorted tension-pairs view — keeps the provenance diff
    # deterministic across re-runs.
    top_tensions = sorted(
        tension_counter.items(),
    ...
    return {
        ...
        "tension_findings": tension_findings,
        "single_block_findings": single_block_findings,
        "tension_counts": [
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Smoke for L4 audit-business-alignment on one enriched cluster. "
    ...
    native_stem = f"l4_verdicts_audit_business_alignment_{cluster_stem}{suffix}"
