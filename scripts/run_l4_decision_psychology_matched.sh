#!/usr/bin/env bash
# Run the L4 decision-psychology smoke across a matched-model × modality grid.
#
# Structurally identical to run_l4_accessibility_matched.sh — only the
...
SMOKE="scripts/smoke_l4_decision_psychology_multimodal.py"
...
for model in "${MODELS[@]}"; do
    ...
    for modality in "${MODALITIES[@]}"; do
        ...
        target="data/derived/l4_audit/audit_decision_psychology/l4_verdicts_audit_decision_psychology_cluster02${suffix}.provenance.json"
...
ls -1 data/derived/l4_audit/audit_decision_psychology/*.provenance.json | sort
