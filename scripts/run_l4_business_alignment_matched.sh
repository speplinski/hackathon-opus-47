#!/usr/bin/env bash
# Run the L4 business-alignment smoke across a matched-model × modality grid.
#
# Structurally identical to run_l4_decision_psychology_matched.sh — only
# the smoke script, output directory, and filename stem differ. By
# default skips runs with an existing audited=1 provenance file; pass
# --all to force-rerun. Every run is a LIVE Anthropic call (no replay
# cache) — expected total: ~6 × (14k in + 2.5k out) ≈ $0.50-0.90.
...
SMOKE="scripts/smoke_l4_business_alignment_multimodal.py"
...
for model in "${MODELS[@]}"; do
    ...
    for modality in "${MODALITIES[@]}"; do
        ...
        target="data/derived/l4_audit/audit_business_alignment/l4_verdicts_audit_business_alignment_cluster02${suffix}.provenance.json"
...
ls -1 data/derived/l4_audit/audit_business_alignment/*.provenance.json | sort
