"""One-shot smoke for the L4 UX-architecture audit — text or multimodal.

Companion to the production ``l4_audit_ux_architecture`` module
(Garrett's Elements of User Experience lens — strategy, scope,
structure, skeleton, surface). Takes one enriched cluster, calls the
Anthropic SDK directly with either a plain text user turn or a
text+image user turn, parses the response through the same parser the
module uses, and writes verdicts / native / provenance next to the
module's own outputs. Zero cache interaction.
...
import and provenance shape (product-type histogram + decision-mode
histogram + unconscious-decision gauge instead of posture / user-tier /
excise-type fields) differ.
...
from auditable_design.layers.l4_audit_ux_architecture import (  # noqa: E402
    DIMENSION_KEYS,
    MAX_TOKENS,
    SKILL_ID,
    SYSTEM_PROMPT,
...
# Decision modes that trip the SKILL.md cap — mirror of the same
# constant in the production module. Factored here too so the smoke's
# provenance gauge and the module's provenance gauge count identical
# findings.
_UNCONSCIOUS_DECISION_MODES = frozenset({"default", "mimicry", "fiat"})

DEFAULT_INPUT = (
    _REPO_ROOT
    / "data/derived/l4_audit/audit_ux_architecture/audit_ux_architecture_input.jsonl"
)
DEFAULT_SCREENSHOT = _REPO_ROOT / "data/artifacts/ui/duolingo_streak_modal.png"
DEFAULT_OUT_DIR = _REPO_ROOT / "data/derived/l4_audit/audit_ux_architecture"
...
def _build_provenance(
    *,
    ...
    smoke vs. prod without adapter code. Garrett-specific:
    product_type histogram (4 keys) + decision_mode histogram (5 keys)
    + unconscious-decision gauge, instead of Cooper's posture / tier /
    excise fields.
    """
    ...
    product_type_hist: dict[str, int] = {p: 0 for p in VALID_PRODUCT_TYPES}
    decision_mode_hist: dict[str, int] = {d: 0 for d in VALID_DECISION_MODES}
    unconscious_decision_findings = 0
    ...
    if payload is not None:
        ...
        for f in payload["findings"]:
            ...
            product_type_hist[product_type] = (
                product_type_hist.get(product_type, 0) + 1
            )
            decision_mode = f["decision_mode"]
            decision_mode_hist[decision_mode] = (
                decision_mode_hist.get(decision_mode, 0) + 1
            )
            if decision_mode in _UNCONSCIOUS_DECISION_MODES:
                unconscious_decision_findings += 1
    ...
    return {
        ...
        "product_type_histogram": product_type_hist,
        "decision_mode_histogram": decision_mode_hist,
        "unconscious_decision_findings": unconscious_decision_findings,
        "fallback_reasons": (
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Smoke for L4 audit-ux-architecture on one enriched cluster. "
    ...
    native_stem = f"l4_verdicts_audit_ux_architecture_{cluster_stem}{suffix}"
