...
**Implementation note:** the clustering and labeling phases are split into two adjacent implementation layers — `l3_cluster` (deterministic, no Claude, writes placeholder labels) and `l3b_label` (Claude-backed, reads L3 output, rewrites labels) — to keep replay-log immutability clean. See `ARCHITECTURE.md §4.4` for the contract.
